...
raster_crs <- readRDS("/raster_crs.RData")
...
ggplot() +
  ...
  geom_sf(data = Coordinates_Group_laea, aes(color = as.factor(Group)), size = 1, color = "black") +
  coord_sf() +
  theme_minimal() +
...
### Save the table created on your computer:
setwd("")
